...
- **Priority** (high to low: Urgent → High → Normal → Low) controls pack order. Within the same level, jobs with a **preferred start** (from drag or the editor) **reserve** those calendar days first; **auto-placed** jobs (no preferred start) then fill **earlier** free work days up to those reservations so the near-term calendar is not left empty before a pinned job. Among auto-placed work, **oldest first by add time** (`addedAtMs`). Among preferred starts, **earlier** time first (then stable id).
...
- **High**: auto-planned jobs with no preferred start are **deferred until about 14 calendar days after the job was added**. While that deferral window is still in the future, those jobs pack **after** other eligible work (so they do not clear the near-term window for lower tiers); once the nominal start is no longer in the future, they follow the usual High-before-Normal ordering. Set a **preferred start** (or drag in week or month view) to start earlier.
